...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Profile
        fields = ['url', 'id', 'user', 'image','phone', 'present_address','permanent_address','city', 'state','zip_code']
        ...
        if request_method == 'POST':
            if password == None:
                ...
                if password != None and old_password == None:
                    raise serializers.ValidationError({"info": "Please provided the old password."})
        return data
    class Meta:
        model = User
